...
```mermaid
flowchart TD
    T["📘 <b>THESIS</b><br/>Chiết khấu hội tụ là cơ hội; mua đất + giữ USD = tự bảo hiểm"]:::thesis
    A["⚔️ <b>ANTITHESIS</b><br/>Chiết khấu là rủi ro thanh khoản/pháp lý; USD không bảo hiểm vị thế VND"]:::antithesis
    X["🔍 <b>CROSS-EXAMINATION</b><br/>3 premises · 12-fallacy audit"]:::probe
    C["🌀 <b>TENSION</b><br/>Dòng hội tụ (chắc chắn, không cần thoát) vs Cổ hội tụ (đầu cơ, cần thoát + chịu rủi ro tiền tệ)"]:::tension
    S["💎 <b>SYNTHESIS (AUFHEBEN)</b><br/>Đất nước là nguồn lợi suất, công cụ là nơi chịu rủi ro"]:::synthesis
    R["🌊 <b>RADIATING IMPLICATION</b><br/>Khóa vị thế = bán quyền thoát; định giá khoản chiết khấu theo lối thoát"]:::ripple

    T -->|excavated premises| X
    X -->|surviving edge cases| A
    T -.->|collides with| C
    A -.->|collides with| C
    C ==>|resolves via Aufheben| S
    S -->|opens door to| R

    classDef thesis     fill:#E1E9F5,stroke:#4A6FA5,stroke-width:2px,color:#2A4B7F
    classDef antithesis fill:#F5E0E0,stroke:#A34A4A,stroke-width:2px,color:#7A2A2A
    classDef probe      fill:#FEEBC8,stroke:#B7791F,stroke-width:1px,color:#7B4B0F
    classDef tension    fill:#F5F1EA,stroke:#6B7280,stroke-width:1px,color:#1A1A2E
    classDef synthesis  fill:#ECE5F2,stroke:#8A6FA5,stroke-width:3px,color:#5D4478
    classDef ripple     fill:#DEECFB,stroke:#2563EB,stroke-width:1px,color:#1E3A8A
```
...
```mermaid
flowchart LR
    A["<strong>DÒNG HỘI TỤ (FLOW)</strong><br/>Tiêu dùng phi mậu dịch rẻ → tiết kiệm USD dồi dào. Gần như chắc chắn. Không cần thoát."]
    B["<strong>CỔ HỘI TỤ (STOCK)</strong><br/>Mua đất/nhà chờ tăng giá. Đầu cơ. Cần lối thoát hợp pháp + quản trị tiền tệ."]
    C["<strong>GIẢI PHÁP CẤU TRÚC</strong><br/>Thu hoạch dòng bằng tiêu dùng rẻ; thu hoạch cổ bằng công cụ có lối thoát + khớp tiền tệ — không trộn hai cược."]
    A -->|conflicts with| B
    A -.->|resolved by| C
    B -.->|resolved by| C
    style C fill:#EDE9FE
